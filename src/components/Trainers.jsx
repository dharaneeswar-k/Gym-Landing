        ...
        >
            ...
            <div className="absolute bottom-0 left-0 w-full p-5 sm:p-8 z-20 flex flex-col justify-end h-full pointer-events-none" style={{ transform: "translateZ(80px)" }}>
                <div className="transform sm:translate-y-6 group-hover:translate-y-0 transition-transform duration-500 ease-out">
                    ...
                    <div className="flex items-center gap-2 mb-4 bg-dark-900/80 backdrop-blur border border-white/10 px-3 py-1.5 sm:py-2 rounded-xl inline-flex">
                        <Award className="w-4 h-4 sm:w-5 sm:h-5 text-yellow-500" />
                        <span className="text-white text-[10px] sm:text-xs font-bold uppercase tracking-wider">{trainer.achievements}</span>
                    </div>

                    {/* Mobile Tap Hint / Desktop Hover Hint */}
...
const Trainers = () => {
    ...
    return (
        <section id="trainers" className="py-20 sm:py-32 bg-black relative overflow-hidden">
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none opacity-[0.02] sm:opacity-[0.03] select-none z-0">
                <h2 className="text-[10rem] sm:text-[15rem] leading-none font-black text-white whitespace-nowrap blur-[4px] sm:blur-[8px]">COACHES</h2>
            </div>
                    ...
                    >
                        <div>
                            <div className="inline-flex items-center justify-center gap-2 mb-3">
                                ...
                                <h2 className="text-primary font-bold uppercase tracking-[0.2em] text-xs sm:text-sm mt-0.5">The Titans Engine</h2>
                                <Star className="w-4 h-4 text-primary fill-primary" />
                            </div>
                            <h3 className="text-3xl sm:text-5xl md:text-6xl font-black uppercase text-white drop-shadow-[0_0_15px_rgba(255,255,255,0.1)]">Meet The Masters</h3>
                        </div>
                    </motion.div>
                </div>

                {/* Horizontal Mobile Scroll Row / Desktop Grid */}
                <div className="flex md:grid md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-12 w-full overflow-x-auto md:overflow-visible snap-x snap-mandatory px-6 sm:px-0 pb-10 md:pb-0 hide-scroll-mobile">
                ...
                {selectedTrainer && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6">
                        ...
                        <motion.div
                            ...
                            className="relative w-full max-w-5xl bg-dark-900 border border-dark-700 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row max-h-[90vh]"
                        >
                            <button
                                onClick={() => setSelectedTrainer(null)}
                                className="absolute top-4 right-4 z-50 bg-black/50 backdrop-blur p-2 rounded-full border border-white/10 hover:bg-primary hover:border-primary transition-colors text-white"
                            >
                                <X className="w-6 h-6" />
                            </button>
                            ...
                            <div className="md:w-7/12 p-6 sm:p-10 lg:p-12 overflow-y-auto hidden-scrollbar z-20">
                                <h4 className="text-sm font-bold text-primary uppercase tracking-[0.2em] mb-2">{selectedTrainer.role}</h4>
                                <h3 className="text-4xl sm:text-5xl font-black uppercase text-white mb-6 leading-none shadow-sm">{selectedTrainer.name}</h3>

                                <div className="flex flex-wrap items-center gap-3 mb-8">
                                    <span className="bg-dark-800 text-white border border-dark-600 px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest flex items-center gap-2">
                                ...
                                <div className="bg-dark-800 rounded-2xl p-6 border border-dark-700 flex flex-wrap gap-6 justify-between items-center relative overflow-hidden group hover:border-primary/50 transition-colors">
